...
### 新增
- Telegram Bot 集成（可选）：通过 Telegram 管理会话 — 创建、列表、切换、停止、恢复、审批权限
- 命令：`/new`、`/list`、`/switch`、`/stop`、`/resume`、`/status`
- 用户白名单：通过 `TELEGRAM_ALLOWED_USERS` 环境变量配置（默认拒绝所有用户）
- 停止生成：生成时发送按钮变为停止按钮，一键中断 Claude 响应
